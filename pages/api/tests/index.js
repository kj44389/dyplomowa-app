...
const getAllByTestsIds = async (tests) =>{
    ...
    if (results.length == 0) return { status: 404, message: 'Tests not found2' };
...
export default async (req, res) => {  
  try {
    ...
    if(req.query.by === 'test_id' && req.query.tests) 
        response = await getAllByTestsIds(JSON.parse(req.query.tests));
