...
export async function getServerSideProps(context) {
	...
	let questionsDataTemp = await fetch(`${absoluteUrlPrefix}/api/v2/questions?test_id=${testId}`).then((response) =>
		response.json()
	);
	const questionsIds = [];
	let index = 1;
	...
	let answersDataTemp = await fetch(`${absoluteUrlPrefix}/api/v2/answers/${JSON.stringify(questionsIds)}`).then(
...
function NewTest({ fetched_test, fetched_answers, fetched_questions, error }) {
	...
	async function handleSubmit() {
		...
		if (router.query.edit) {
			const res = fetch(`${absoluteUrlPrefix}/api/v2/test/${test.test_id}`, { method: "PATCH", body: body });
		} else {
			const res = fetch(`${absoluteUrlPrefix}/api/v2/test/${test.test_id}`, { method: "POST", body: body });
		}
