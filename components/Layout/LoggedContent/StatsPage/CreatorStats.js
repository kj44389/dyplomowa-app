...
const CreatorStats = () => {
	const [worstQuestion, setWorstQuestion] = useState(null);
		...
		<div className='flex w-full max-w-xs flex-col md:max-w-md'>
			<div className='m-2 w-full max-w-xs rounded-2xl bg-gray-800 p-4 text-red-400 shadow-lg md:max-w-md'>
				...
				<div className='flex flex-col justify-start space-y-2 p-2'>
					...
					<p className='text-left font-bold  text-gray-100 '>
						Average score: <span className='text-md font-thin'>{((questionStats?.testStats[0]?.avgscore / questionStats?.testStats[0]?.avgtotal) * 100).toFixed(0)}%</span>
					</p>
					<p className='text-left font-bold  text-gray-100 '>
						Hardest question:{' '}
					...
					<p className='text-left font-bold  text-gray-100 '>
						The best score:{' '}
						<span className='text-md font-thin'>
							{questionStats?.testDone[0]?.user_full_name} - {((questionStats?.testDone[0]?.points_scored / questionStats?.testDone[0]?.points_total) * 100).toFixed(0)}%
						</span>
					</p>
				</div>
			</div>
