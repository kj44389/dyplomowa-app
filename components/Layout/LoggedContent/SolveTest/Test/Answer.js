...
const Answer = ({ answer, index, question_id, disabled, picked, onClick }) => {
	...
	return (
		<div className="my-1 flex w-full flex-row items-center p-3">
			<>
				<div className="text-md text-white-500 m-2 flex h-8 w-8 cursor-pointer items-center justify-center rounded-md bg-gray-800/30 p-4">
					{index + 1}
				</div>
				<div className="flex flex-row md:items-end md:justify-center">
					<label className="label ">
						<input
							...
							disabled={disabled}
							onClick={(e) => onClick(question_id, answer_id)}
						/>
					</label>
				</div>
			</>
